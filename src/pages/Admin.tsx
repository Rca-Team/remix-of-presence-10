...
import { User, Calendar, Filter, Clock, UserPlus, FolderKanban, School, GraduationCap, Menu, LayoutDashboard, Settings, Image, FileText, Bell, Users, BarChart3, Sparkles, Shield, Activity, TrendingUp, ChevronRight, Download, Send, Zap, Globe, UserCog, CreditCard } from 'lucide-react';
...
const Admin = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const {
        data: faceData,
        error: faceError
      } = await supabase.from('attendance_records').select('id, user_id, device_info').eq('status', 'registered');
      if (!faceError && faceData && faceData.length > 0) {
        const uniqueFaces = faceData.reduce((acc: {
          id: string;
          name: string;
          employee_id: string;
        }[], record) => {
      ...
      const {
        data: todayData
      } = await supabase.from('attendance_records').select('user_id, status, device_info').gte('timestamp', `${today}T00:00:00`).lte('timestamp', `${today}T23:59:59`).neq('status', 'registered');
      if (todayData) {
  ...
  useEffect(() => {
    ...
    const adminChannel = supabase.channel('admin-dashboard').on('postgres_changes', {
      event: '*',
      schema: 'public',
        ...
        <PullToRefresh onRefresh={handleRefresh} enabled={isMobile} className="relative">
          <div className="relative mobile-container py-4 sm:py-6">
            ...
            y: 0
              ...
              {isMobile && <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
                  ...
                  <SheetContent side="right" className="w-80 p-0 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border-l border-blue-100 dark:border-blue-900">
                    ...
                    <div className="py-2">
                      {mobileNavItems.map((item, i) => <motion.button key={item.id} initial={{
                    opacity: 0,
                    x: 20
                  }} animate={{
                    opacity: 1,
                    ...
                    delay: i * 0.05
                  }} onClick={() => handleTabChange(item.id)} className={`w-full flex items-center gap-3 px-4 py-3.5 text-left transition-all ${activeTab === item.id ? 'bg-blue-50 dark:bg-blue-950/50 border-l-4 border-blue-500' : 'hover:bg-muted/50'}`}>
                          ...
                          <div className="flex-1">
                            ...
                            {item.badge && <span className="text-xs text-blue-500">New updates</span>}
                          </div>
                          {item.count !== undefined && item.count > 0 && <Badge className="bg-red-500 text-white">{item.count}</Badge>}
              ...
              {!isMobile && <div className="flex items-center gap-2">
                  ...
                  <Select value={nameFilter} onValueChange={setNameFilter}>
                    ...
                    <SelectContent>
                      ...
                      {availableFaces.map(face => <SelectItem key={face.id} value={face.id}>
                          {face.name}
                        </SelectItem>)}
                    </SelectContent>
                  </Select>
                  ...
                  <AnimatedNotificationBadge count={notificationCount}>
                    <Button variant="outline" size="icon" onClick={() => handleTabChange('notifications')} className="bg-white/70 dark:bg-slate-800/70 backdrop-blur border-blue-100 dark:border-blue-900">
                  ...
                  <BulkNotificationService availableFaces={availableFaces} />
                  <AttendanceExport />
                  <Button variant="outline" onClick={() => setViewMode(viewMode === 'grid' ? 'list' : 'grid')} className="bg-white/70 dark:bg-slate-800/70 backdrop-blur border-blue-100 dark:border-blue-900">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              ...
              delay: 0.15
                  ...
                  <TabsList className="mb-6 flex-wrap gap-1 p-1.5 bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl border border-blue-100 dark:border-blue-900/50 rounded-xl shadow-lg">
                    ...
                    <TabsTrigger value="faces" className="flex items-center gap-2 data-[state=active]:bg-gradient-to-r data-[state=active]:from-blue-600 data-[state=active]:to-blue-500 data-[state=active]:text-white rounded-lg">
                      ...
                      <span>All Faces</span>
                      {attendanceUpdated && <span className="flex h-2 w-2 relative">
              ...
              {/* Mobile filter bar */}
              {isMobile && activeTab === 'faces' && <motion.div initial={{
              opacity: 0,
              y: 10
            }} animate={{
              opacity: 1,
              ...
              <AnimatePresence mode="wait">
                <TabsContent value="principal" className="space-y-4 mt-0">
                  ...
                  y: -20
                    ...
                    {/* Mobile Stats */}
                    {isMobile && <div className="grid grid-cols-2 gap-3 mb-4">
                        {statsCards.map((stat, i) => <motion.div key={i} initial={{
                      opacity: 0,
                      scale: 0.9
                    }} animate={{
                      opacity: 1,
                ...
                <TabsContent value="faces" className="space-y-4 mt-0">
                  ...
                  y: -20
                    ...
                    <AdminFacesList viewMode={isMobile ? 'grid' : viewMode} selectedFaceId={selectedFaceId} nameFilter={nameFilter} setSelectedFaceId={id => {
                    haptic('selection');
