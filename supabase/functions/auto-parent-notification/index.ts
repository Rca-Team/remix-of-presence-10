...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Parent info:', { parentEmail, parentName, hasPhone: !!parentPhone });
...
<tr><td style="background:linear-gradient(135deg,#3b82f6 0%,#1d4ed8 100%);padding:30px;text-align:center;">
<h1 style="color:#ffffff;margin:0;font-size:24px;">📚 School Attendance System</h1></td></tr>
<tr><td style="padding:20px;text-align:center;">
<span style="display:inline-block;background-color:${statusColor};color:white;padding:8px 20px;border-radius:20px;font-weight:600;font-size:14px;">${statusEmoji} ${statusBadge}</span></td></tr>
<tr><td style="padding:20px 30px;">
<p style="color:#374151;margin:0 0 10px 0;">Dear ${escapeHtml(parentName)},</p>
<p style="color:#4b5563;">Your child <strong>${escapeHtml(studentName)}</strong> has been marked as <strong style="color:${statusColor};">${status.toUpperCase()}</strong> for today's attendance.</p></td></tr>
<tr><td style="padding:0 30px 20px 30px;">
<table width="100%" style="background-color:#f9fafb;border-radius:8px;padding:15px;">
<tr><td><p style="margin:0;color:#6b7280;font-size:12px;text-transform:uppercase;">Student</p><p style="margin:5px 0 0 0;color:#111827;font-weight:600;">${escapeHtml(studentName)}</p></td>
<td align="right"><p style="margin:0;color:#6b7280;font-size:12px;text-transform:uppercase;">Date & Time</p><p style="margin:5px 0 0 0;color:#111827;font-weight:600;">${currentDate}<br/>${currentTime}</p></td></tr></table></td></tr>
<tr><td style="background-color:#f9fafb;padding:20px 30px;text-align:center;border-top:1px solid #e5e7eb;">
<p style="margin:0;color:#6b7280;font-size:12px;">This is an automated message from the School Attendance System.</p></td></tr>
    ...
    // ============ 2. SEND WHATSAPP ============
    if (parentPhone) {
      const whatsappMsg = status === 'present'
        ? `✅ Dear ${parentName}, your child ${studentName} has arrived at school at ${time}. Have a great day! - Presence`
        : status === 'late'
        ? `⏰ Notice: ${studentName} arrived late at school at ${time} today. Please ensure timely arrival. - Presence`
        : `❌ Alert: ${studentName} has been marked absent today (${date}). If unexpected, please contact the school. - Presence`;

      const waResult = await sendWhatsAppMessage(parentPhone, whatsappMsg);
      results.whatsappSent = waResult.success;
      if (!waResult.success && waResult.error) {
        results.errors.push(`WhatsApp: ${waResult.error}`);
      }
      ...
      // ============ 3. FALLBACK: SEND SMS if WhatsApp fails ============
      if (!waResult.success) {
